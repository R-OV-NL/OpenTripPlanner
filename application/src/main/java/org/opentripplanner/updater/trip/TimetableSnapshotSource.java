...
public class TimetableSnapshotSource implements TimetableSnapshotProvider {
    ...
    private Result<UpdateSuccess, UpdateError> handleScheduledTrip(
        ...
        if (trip == null) {
            debug(tripId, serviceDate, "No trip found for tripId, skipping TripUpdate.");
        }

        final FeedScopedId serviceId = trip.getServiceId();
            ...
            if (stopTimeUpdate.getScheduleRelationship() != StopTimeUpdate.ScheduleRelationship.SKIPPED) {
                ...
                if (stopTimeUpdate.hasArrival() && stopTimeUpdate.getArrival().hasTime()) {
                    ...
                    debug(
                            tripId,
                            serviceDate,
                            "Trip update misses arrival time, setting the arrival to the departure."
                    );
    ...
    private Result<UpdateSuccess, UpdateError> handleAddedTrip(
        ...
        var tripHeadSign = getHeadsignFromTripStopTimeUpdates(tripId.getFeedId(), stopTimeUpdates);

        if(tripHeadSign != null)
            tripBuilder.withHeadsign(tripHeadSign);
    ...
    @Nullable
    private I18NString getHeadsignFromTripStopTimeUpdates(String feedId, List<StopTimeUpdate> stopTimeUpdates) {
        var lastStop = stopTimeUpdates.getLast();

        if (lastStop.hasStopId()) {
            var stopId = lastStop.getStopId();

            //Find the stop in the graph
            var stop = transitEditorService.getRegularStop(new FeedScopedId(feedId, stopId));
    ...
    private String getRouteShortNameFromTripDescriptor(TripDescriptor tripDescriptor) {
        FeedScopedId routeId = new FeedScopedId(tripDescriptor.getTripId(), tripDescriptor.getRouteId());

        var splitRouteId = routeId.getId().split("_");
        var routeShortName = splitRouteId[1];
        ...
        return switch (routeShortName) {
            ...
            case "ST" -> "Stoptrein";
            case "S" -> "Sneltrein";
            case "ICD" -> "Intercity Direct";
            case "ICE" -> "Intercity Express";
            case "SPC" -> "Speciale Trein";
            default -> routeShortName;
        };
    }
    ...
    private Route createRoute(TripDescriptor tripDescriptor, FeedScopedId tripId) {
        ...
        if (
            ...
            var routeShortName = getRouteShortNameFromTripDescriptor(tripDescriptor);
            builder.withShortName(routeShortName);

            var routeLongName = addedRouteExtension.routeLongName();
            builder.withLongName(new NonLocalizedString(routeLongName));

            builder.withGtfsType(addedRouteExtension.routeType());
            var mode = TransitModeMapper.mapMode(addedRouteExtension.routeType());

            builder.withMode(Objects.requireNonNullElse(mode, TransitMode.BUS));
    ...
    private Result<UpdateSuccess, UpdateError> addTripToGraphAndBuffer(
        ...
        for (int stopIndex = 0; stopIndex < newTripTimes.getNumStops(); stopIndex++) {
            ...
            // Set realtime and scheduled platform (Duplicate from line 950+)
    ...
    private Result<UpdateSuccess, UpdateError> validateAndHandleModifiedTrip(
        ...
        if (!tripDescriptor.hasStartDate()) {
            ...
            final Set<FeedScopedId> serviceIdsYesterday = transitEditorService
                    .getCalendarService()
    ...
    private Result<UpdateSuccess, UpdateError> handleCanceledTrip(
        ...
        if (!cancelScheduledSuccess) {
            ...
            warnings.add(UpdateSuccess.WarningType.NO_TRIP_FOR_CANCELLATION_FOUND);
        }

        debug(tripId, serviceDate, "Canceled trip");

        var updateSuccess = new UpdateSuccess(warnings, tripId.toString());
